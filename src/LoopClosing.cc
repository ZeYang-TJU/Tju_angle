...
void LoopClosing::SetiGPSFusioner(iGPSFusion *piGPSFusioner)
{
    mpiGPSFusioner = piGPSFusioner;
}

void LoopClosing::LoadiGPSPosition(vector<double> vTimestamps, vector<cv::Point3f> viGPSPosition)
{
...
void LoopClosing::MergeLocal()
{
    ...
    if(pCurrentMap->IsInertial() && pMergeMap->IsInertial()) //TODO Check the correct initialization
    {
        ...
        while(pKFi && nInserted < numTemporalKFs)
        {
            ...
            spLocalWindoiGPS.insert(spMPi.begin(), spMPi.end());
        }
    ...
    for(KeyFrame* pKFi : spLocalWindowKFs)
    {
        ...
        spLocalWindoiGPS.insert(spMPs.begin(), spMPs.end());
    }
    ...
    for(MapPoint* pMPi : spLocalWindoiGPS)
    {
        if(!pMPi || pMPi->isBad())
            continue;
...
void LoopClosing::RunGlobalBundleAdjustment(Map* pActiveMap, unsigned long nLoopKF)
{
    ...
    if(!bImuInit)
        Optimizer::GlobalBundleAdjustemnt(pActiveMap,20,&mbStopGBA,nLoopKF,false);
    else
