    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                {
                    if (mbMonocular)  //Monocular or stereo
...
void LocalMapping::InitializeiGPSDir(vector<Eigen::Matrix4d>& Tcw)
{
    ...
    //unique_lock<mutex> lock(mpAtlas->GetCurrentMap()->mMutexMapUpdate);
    //mpAtlas->GetCurrentMap()->ApplyScaledRotation(rMatrix,mScaleiGPS,true,tMatrix);
    //mpTracker->UpdateFrameiGPS(mScaleiGPS,mpCurrentKeyFrame);
    //mpCurrentKeyFrame->GetMap()->IncreaseChangeIndex();

    //FulliGPSBA
    //Optimizer::FulliGPSBA(mpAtlas->GetCurrentMap(), 100,false, 0, NULL, true);
